 ...
 * @ Fehlerbehandlung: sieht nach ob es 0 ^0 ist
 * Wenn eingabe OK wird sie an Rek weitergegeben
 */
 ...
 return potenzRech(basis, exponent);

 }
 /* Eigentliche Rechnung des Potenzprogramms
  * kann aber nur ueber potzen gestatrtet werden
  * Giebt double aus
  * liest int und double ein
  * ruft sich selbst auf
  */

 private static double potenzRech(double basis, int exponent){

   if(exponent == 0){
     return 1;
   }
   if(exponent < 0){
     return potenzRech(basis, exponent + 1) / basis;
   }else
   return potenzRech(basis, exponent - 1) * basis;
 
 }

/* Main fier die Benutzerfuehrung
 * Nutzer kann verschiedene Optionen wahlen
 * Eingaben werden auf gueltigkeit geprueft
 */
  ...
  public static void main(String[] argv) {
   ...
   IO.println("Drücke 4 um das Programm zu beenden");
